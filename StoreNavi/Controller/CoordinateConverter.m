...
+ (EastSouthDistance)metersFromPoint:(const MKMapPoint)fromAnchorMKPoint toPoint:(const MKMapPoint)to {
    CLLocationDistance metersPerMapPoint = MKMetersPerMapPointAtLatitude(MKCoordinateForMapPoint(fromAnchorMKPoint).latitude);

    EastSouthDistance eastSouthDistance = {
...
- (instancetype)initWithAnchors:(GeoAnchorPair)anchors {
    ...
    if (self) {
        ...
        EastSouthDistance hyp = [CoordinateConverter metersFromPoint:_fromAnchorMKPoint toPoint:toAnchorMapkitPoint];
...
- (CGPoint)pointFromCoordinate:(CLLocationCoordinate2D)coordinate {
    ...
    EastSouthDistance toFix = [CoordinateConverter metersFromPoint:self.fromAnchorMKPoint toPoint:MKMapPointForCoordinate(coordinate)];
